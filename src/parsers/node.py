...
from models import Node, NodeType, NodeScope
...
class _NodeParser:
    ...
    def _create_node(self, ts_node: TsNode, node_type: NodeType, parent_id: Optional[str]) -> Optional[Node]:
        ...
        if parent_id and parent_id != f"{self.filepath}::module::{self.module_path}":
            ...
            node_id = f"{self.filepath}::{node_type.value.lower()}::{parent_name}.{name}"
    ...
    def _determine_scope(self, node_type: NodeType, parent_id: Optional[str]) -> NodeScope:
        ...
        if "::class::" in parent_id:
            return NodeScope.METHOD if node_type == NodeType.FUNCTION else NodeScope.CLASS
        elif "::function::" in parent_id:
            return NodeScope.LOCAL
        else:
            return NodeScope.MODULE
    
    def _build_qualified_name(self, name: str, node_type: NodeType, parent_id: Optional[str]) -> str:
